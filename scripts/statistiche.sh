...
echo -e  "${COLOR_GREEN}Number of successful read operations:${COLOR_STANDARD} ${n_reads}"
if [ ${n_reads} -gt 0 ]; then
    mean_bytes_read=$(echo "scale=4; ${tot_bytes_read} / ${n_reads}" | bc -l)
fi

echo -e "${COLOR_GREEN}Total number of bytes read by clients:${COLOR_STANDARD} $(( tot_bytes_read + tot_bytes_readNFiles))"
if (( n_reads + n_readNFiles > 0 )); then
    mean_bytes_gen_read=$(echo "scale=4; ($tot_bytes_read) / ($n_reads)" | bc -l )
    echo -e "${COLOR_GREEN}Average number of bytes read by clients:${COLOR_STANDARD} ${mean_bytes_gen_read}"
fi
...
echo -e  "${COLOR_GREEN}Number of successful write operations:${COLOR_STANDARD} ${n_write}"
if [ ${n_write} -gt 0 ]; then
  mean_bytes_written=$(echo "scale=4; ${tot_written_bytes} / ${n_write}" | bc -l)
fi


echo -e "${COLOR_GREEN}Total number of bytes written:${COLOR_STANDARD} $(( tot_written_bytes + tot_appended_bytes))"
if (( n_appends + n_write > 0 )); then
    ...
    echo -e  "${COLOR_GREEN}Average number of bytes written:${COLOR_STANDARD} ${average_written_bytes}"
fi

echo ""

echo -e "${COLOR_GREEN}Number of successful append operations:${COLOR_STANDARD} ${n_appends}"
if [[ ${n_appends} -gt 0 ]]; then
  average_appended_bytes=$(echo "scale=4; ${tot_appended_bytes} / ${n_appends}" | bc -l)
  echo -e "${COLOR_GREEN}Average number of bytes appended:${COLOR_STANDARD} ${average_appended_bytes}"
fi
...
n_open=$(grep " opened file " -c $LOG_FILE)
...
echo -e  "${COLOR_GREEN}Number of successful create operations:${COLOR_STANDARD} $n_create"
echo -e  "${COLOR_GREEN}Number of successful open operations:${COLOR_STANDARD} $n_open"
echo -e  "${COLOR_GREEN}Number of successful create-lock operations:${COLOR_STANDARD} $n_createlocks"
echo -e  "${COLOR_GREEN}Number of successful open-lock operations:${COLOR_STANDARD} $n_openlocks"
echo -e  "${COLOR_GREEN}Number of successful lock operations:${COLOR_STANDARD} $n_locks"
echo -e  "${COLOR_GREEN}Number of successful unlock operations:${COLOR_STANDARD} $n_unlocks"
echo -e  "${COLOR_GREEN}Number of successful close operations:${COLOR_STANDARD} $n_close"
echo -e  "${COLOR_GREEN}Number of successful cache replacements:${COLOR_STANDARD} $n_eject"


# getting number of requests satisfied by each thread
echo -e "\n${COLOR_GREEN}Number of requests managed by each worker:${COLOR_STANDARD}"
...
echo -e  "${COLOR_GREEN}Max stored data size (Mbytes):${COLOR_STANDARD} $max_size"
echo -e  "${COLOR_GREEN}Max number of stored files:${COLOR_STANDARD} $max_files"
echo -e  "${COLOR_GREEN}Max number of simultaneously connected clients:${COLOR_STANDARD} $max_conn"
